...
sys.path.append(config.pymitools_folder)

import mi_datahandler
...
class TokenGenerator(NetworkSend):
    def __init__(self, port):
        NetworkSend.__init__(self, port, 'localhost', '!d')
        self.running = True
    
    def run(self):
        while self.running:
            time.sleep(self.getNextTime())
            self.send(random.choice([2,-2,3,-3,4,5]))
...
class Snapper:
    ...
    def __init__(self):
        ...
        self.classifier = getattr(mi_datahandler, CLASSIFIER_NAME+'Classifier')('lanefeedback ' + CLASSIFIER_NAME, CLASSIFIER_DIC)
        self.classifier.reset()
        self.classifier.set_select_type('binarySelectorStep') 
    ...
    def event(self, event):
        ...
        if self.classifier.name.lower().find('sim'):
            if event.type==KEYDOWN:         
                if event.key==K_LEFT:
                    self.classifier.new_input(-1)
                elif event.key==K_RIGHT:
                    self.classifier.new_input(1)
                elif event.key==K_DOWN:
                    self.classifier.new_input(2)
            elif event.type==KEYUP:
                if event.key in [K_LEFT, K_RIGHT]:
                    self.classifier.new_input(0)
	...
	dt = dt/1000.0
        ...
        if self.state in [STATE_RUNNING, STATE_PLAYING]:
            ...
            new_data = self.classifier.get_output()
            if new_data != None:
                self.control_value = new_data[0]
...
if __name__ == "__main__":
    print 'getting data via', CLASSIFIER_NAME
    if CLASSIFIER_NAME == 'EPFLonlineEEGServer':
        from eegserverinterface import *
        from epflconfigloader import *
        
	print CLASSIFIER_DIC['classifier']
	print CLASSIFIER_DIC['modality']
	print CLASSIFIER_DIC['host']
        eegbuddy = EEGServerBuddy(CLASSIFIER_DIC['classifier'], CLASSIFIER_DIC['modality'], CLASSIFIER_DIC['host'])
        if not eegbuddy.Start():
            print 'Something went wrong with eegserver start up :('
            sys.exit(-1)
    
    # start sending tokens
    tokenGenerator = TokenGenerator(config.event_port)
    thread.start_new_thread(tokenGenerator.run, ())
